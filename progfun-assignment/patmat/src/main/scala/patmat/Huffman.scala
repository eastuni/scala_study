...
object Huffman {
   ...
   */
    ...
    case Nil => Nil
    case head :: tail => (head,chars.count(_ == head)) :: times(chars.filterNot(head == _))
  }
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs.sortWith((a,b)=>a._2 < b._2).map((f) => Leaf(f._1,f._2))
   ...
   */
    ...
    def encode_helper(tree: CodeTree)(text: List[Char]): List[Bit] = (text,tree) match {
      case (Nil,_) => Nil
      case (a::t,Leaf(c,w)) => encode_helper(tree_source)(t)
      case (a::t,Fork(left,right,chars,w)) if(this.chars(left).contains(a)) => 0::encode_helper(left)(text);
      case (a::t,Fork(left,right,chars,w)) if(this.chars(right).contains(a)) =>  1::encode_helper(right)(text);
    }
